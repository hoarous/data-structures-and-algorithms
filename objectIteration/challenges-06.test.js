...
const hasChildrenValues = (arr, character) => {
  // Solution code here...
  let target = Object.values(arr.find(char => char.name === character));
  return target[2].length > 0;
};
...
const hasChildrenEntries = (arr, character) => {
  // Solution code here...
  let target = Object.entries(arr.find(char => char.name === character));
  return target[2][1].length > 0;
};
...
const houseSize = (arr) => {
  ...
  arr.forEach(character => {
    let family = 1;
    if(character.spouse) family++;
    family += character.children.length;
    sizes.push({house: character.house, members: family});
  });
  return sizes;
};
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach(character => {
    let family = 1;
    if(character.spouse && !(deceasedSpouses.indexOf(character.spouse) + 1)) family++;
    family += character.children.length;
    survivors.push({house: character.house, members: family});
  });
  return survivors;
};
