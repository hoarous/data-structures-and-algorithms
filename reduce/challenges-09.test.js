...
const countNumberOfChildren = (arr) => {
  // Solution code here...
  return arr.reduce((acc, cur) => cur.children?acc+=cur.children.length:acc, 0);
};
...
const calculateAverage = (arr) => {
  // Solution code here...
  let nums = arr.reduce((acc, cur) => {
    return {count:acc.count+1,sum:acc.sum+cur};
  }, {count:0,sum:0});
...
const extractStat = (statName, arr) => {
  // Solution code here...
  return arr.reduce((acc, cur) => {
    if(cur.stat.name === statName) return cur;
    return acc;
  }, null);
};
...
const extractChildren = (arr) => {
  // Solution code here...
  return arr.filter(char => char.name.indexOf('a') + 1).reduce((acc, cur) =>{
    if(cur.children){
      cur.children.forEach(child => acc.push(child));
    }
    return acc;
  }, []);
};
