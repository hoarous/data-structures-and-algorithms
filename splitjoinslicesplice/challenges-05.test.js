...
const howMuchPencil = (str) => {
  let result = [];
  // Solution code here...
  for(let i = 0; i <= str.length; i++)result.push(str.slice(i,str.length));
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(ingredient =>{
    let item = ingredient.slice(ingredient.indexOf(' ') + 1, ingredient.length);
...
const removeEvenValues = (arr) => {
  // Solution code here...
  let evens = arr.findIndex(num => num%2===0);
  while(evens+1){
    arr.splice(evens, 1);
    evens = arr.findIndex(num => num%2===0);
  }
};
...
const totalSumCSV = (str) => {
  let total = 0;
  // Solution code here...
  str.split(',').forEach(val => total += parseInt(val));
  return total;
};
...
const extractVowels = (str) => {
  // Solution code here...
  let regexp = /[aeiou]/ig;
  return [str.split(regexp).join(''), str.match(regexp).sort().join('')];
};
