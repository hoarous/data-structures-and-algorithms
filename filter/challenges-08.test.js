...
const filterStringsWithVowels = (arr) => {
  // Solution code here...
  return arr.filter(str => str.match(/[aeiou]/ig));
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  ...
  for(let i = 0; i < forbiddenValues.length; i++){
    newArr = newArr.filter(val => val!==forbiddenValues[i]);
  }
  return newArr;
};
...
const getStatName = (arr, minBaseStat) => {
  // Solution code here...
  return getBaseStatGreaterThan(arr, minBaseStat).map(stat => stat.stat.name);
};
...
const evenOddNumericValues = (arr) => {
  // Solution code here...
  return arr.filter(val => Number.isInteger(val)).map(num => ((num%2) ? 'odd' : 'even'));
};
