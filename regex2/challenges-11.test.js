...
const validateEmail = (email) => {
  // Solution code here...
  return email.match(/^[\w\d]+(.[\w\d]+)?@[\w\d]+.((net)|(com)|(org))$/);
};
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  return phoneNumber.toString().match(/^((\(\d{3}\))|(\d{3}))( |-)?\d{3}( |-)?\d{4}$/);
};
...
const findTagNames = elements => {
  ...
  return elements.reduce((arr, element) => {
    element.match(/\/\w+/g).forEach(tag => arr.push(tag));
    return arr;
  }, []);
};
